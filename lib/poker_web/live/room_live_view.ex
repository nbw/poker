defmodule PokerWeb.RoomLiveView do
  ...
  def handle_event("card",
    ...
    %{assigns: %{room_id: room_id, user_token: user_token}} = socket)
  do
    {num, _} = Integer.parse(num) # Convert to integer
  ...
  def handle_event("task_title", %{"title" => %{"query" => query}},
  ...
  def handle_event("task_url", %{"url" => %{"query" => query}},
    %{assigns: %{room_id: room_id}} = socket)
  do
